...
import { register, googleLogin, clearError, setAuthError } from '../redux/authSlice';
import { useNavigate, Link } from 'react-router-dom';
import { useGoogleLogin } from '@react-oauth/google';
import { User, Mail, Lock, Loader, QrCode, ArrowLeft, Eye, EyeOff } from 'lucide-react';
import qrvibeLogoMark from '../assets/qrvibe-logo-mark.svg';
...
const RegisterPage = () => {
    ...
    const handleGoogleLogin = useGoogleLogin({
        ...
        onError: (errorResponse) => {
            console.error('Google Registration Failed:', errorResponse);
            dispatch(setAuthError('Google registration was cancelled or failed to initialize.'));
        }
    });
        ...
        <div className="w-full p-4 sm:p-6 min-h-screen flex items-center justify-center relative bg-slate-50 font-sans overflow-hidden">
            ...
            <div className="glass max-w-md w-full p-6 sm:p-10 rounded-3xl sm:rounded-[40px] shadow-2xl border border-white/50 relative">
                <div className="text-center mb-8 sm:mb-10">
                    <img src={qrvibeLogoMark} alt="QRVibe" className="w-16 h-16 mx-auto mb-6 object-contain" />
                    <h2 className="text-2xl sm:text-3xl font-extrabold mb-2 text-slate-900">Create Account</h2>
                    <p className="text-slate-500 text-sm">Join the next generation of marketing.</p>
                </div>
                ...
                <button
                    ...
                    className="w-full flex items-center justify-center gap-3 bg-white border border-slate-200 py-3.5 rounded-2xl font-bold text-slate-700 hover:bg-slate-50 transition-all mb-6 shadow-sm disabled:opacity-70 disabled:cursor-not-allowed"
                >
                    {loading ? (
                ...
                <form className="space-y-4" onSubmit={handleSubmit}>
                    <div>
                        ...
                        <div className="relative group">
                            ...
                            <input type="text" value={name} onChange={(e) => setName(e.target.value)} required placeholder="John Doe" className="w-full bg-slate-50 border border-slate-100 pl-11 pr-4 py-3.5 rounded-2xl outline-none focus:border-indigo-500 focus:bg-white transition-all text-sm font-medium text-slate-900" />
                        </div>
                    </div>
                    <div>
                    ...
                    <div className="flex items-center gap-2 px-1 py-2">
                        ...
                        <label htmlFor="terms" className="text-xs text-slate-500">I agree to the <a href="#" className="text-indigo-600 font-bold hover:underline">Terms</a> and <a href="#" className="text-indigo-600 font-bold hover:underline">Privacy Policy</a></label>
                    </div>

                    <button disabled={loading} className="w-full bg-indigo-600 text-white py-4 rounded-2xl font-bold text-lg hover:bg-indigo-700 transition-all shadow-xl shadow-indigo-100 mt-2 flex items-center justify-center gap-2 disabled:opacity-70">
                ...
                <div className="text-center mt-8 pt-6 border-t border-slate-100">
                    <p className="text-sm text-slate-500 font-medium">
                        ...
                        <Link to="/login" className="text-indigo-600 font-bold hover:underline ml-1">Log In</Link>
                    </p>
